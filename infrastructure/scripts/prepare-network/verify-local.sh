...
verify_local_config() {
  ...
  # Interface IPs
  local int_actual
  int_actual=$(ip -4 -o addr show "$INTERNAL_IFACE" 2>/dev/null | grep 'inet ' | awk '{print $4}' | cut -d/ -f1 | head -1)
  ...
  local ext_actual
  ext_actual=$(ip -4 -o addr show "$EXTERNAL_IFACE" 2>/dev/null | grep 'inet ' | awk '{print $4}' | cut -d/ -f1 | head -1)
  ...
  # Default route
  local route_actual
  route_actual=$(ip route show default 2>/dev/null | head -1)
